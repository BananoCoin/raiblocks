...
TEST (rpc, history)
{
	...
	rai::genesis genesis;
<<<<<<< HEAD:banano/core_test/rpc.cpp
	rai::state_block usend (rai::genesis_account, node0->latest (rai::genesis_account), rai::genesis_account, rai::genesis_amount - rai::kBAN_ratio, rai::genesis_account, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
=======
...
>>>>>>> bcc55f99bcdf5c03bd766639c1dcd14bcb6ee56c:rai/core_test/rpc.cpp
	...
	ASSERT_EQ (rai::kBAN_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	...
	ASSERT_EQ (rai::kBAN_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
...
TEST (rpc, payment_wait)
{
	...
	request1.put ("amount", rai::amount (rai::BAN_ratio).to_string_dec ());
	...
	system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::BAN_ratio);
	...
	request1.put ("amount", rai::amount (rai::BAN_ratio * 2).to_string_dec ());
	test_response response3 (request1, rpc, system.service);
	while (response3.status == 0)
	{
		system.poll ();
...
<<<<<<< HEAD:banano/core_test/rpc.cpp
	...
	ASSERT_EQ (boost::str (boost::format ("Banano %1%.%2%") % BANANO_VERSION_MAJOR % BANANO_VERSION_MINOR), response1.json.get<std::string> ("node_vendor"));
=======
	{
		rai::transaction transaction (system.nodes[0]->store.environment, nullptr, false);
...
TEST (rpc, ban_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::BAN_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, ban_from_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "ban_from_raw");
	request1.put ("amount", rai::BAN_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, banoshi_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::banoshi_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, banoshi_from_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "banoshi_from_raw");
	request1.put ("amount", rai::banoshi_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, block_create_state_open)
{
	...
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
<<<<<<< HEAD:banano/core_test/rpc.cpp
	auto send_block (system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::kBAN_ratio));
=======
...
>>>>>>> bcc55f99bcdf5c03bd766639c1dcd14bcb6ee56c:rai/core_test/rpc.cpp
	...
	request.put ("representative", rai::test_genesis_key.pub.to_account ());
	request.put ("balance", rai::kBAN_ratio.convert_to<std::string> ());
